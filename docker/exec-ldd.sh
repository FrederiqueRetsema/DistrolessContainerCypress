...
ldd "${FULL_PATH_HTTPD}" | grep -v "=>" | awk '{print "COPY --from=build "$1" "$1"}' > /build_dir/docker-copy-commands.txt  || exit 1
ldd "${FULL_PATH_HTTPD}" | grep "=>"    | awk '{print "COPY --from=build "$3" "$3"}' >> /build_dir/docker-copy-commands.txt || exit 1
